...
#sed -i 's/network.host: '"ip"'/network.host: '"${host_ip}"'/g' /opt/elasticsearch/config/elasticsearch.yml.j2
#host_name=`hostname`
#sed -i 's/node.name: '"host"'/node.name: '"${host_name}"'/g' /opt/elasticsearch/config/elasticsearch.yml.j2