...
#单节点创建本地表脚本名
local_table_sql=ck_astana_small_local.sql
...
function check_system_tables(){
  ...
  do
    ...
    do
       {
         msg=`clickhouse-client -m  -h ${astana_local_host_name[i]} --port=${tcp_port}  --user=${user} --password=$writer  -q "ATTACH TABLE $table;" 2>&1`
       }||{
            #如果不是表已经存在
            if [[ ${msg:0-17:16} != "already exists.." ]]
            then
               if [[  `clickhouse-client -m  -h ${astana_local_host_name[i]} --port=${tcp_port}  --user=${user} --password=$writer  -q "EXISTS $table;"` == 0  ]]; then
            echo ${astana_local_host_name[i]} $table doesn't exists' and now create it!
            clickhouse-client -m  -h ${astana_local_host_name[i]} --port=${tcp_port}  --user=${user} --password=$writer  -q "${system_table_sqls[$table]}"
              fi
            fi
          }
    done
...
tables="k19_ods.ntc_radius_log_local k19_ods.ntc_voip_log_local k19_ods.ntc_mail_log_local k19_ods.ntc_lbs_log_local k19_ods.ntc_nat_log_local k19_ods.ntc_vpn_log_local k19_ods.dream_http_log_local k19_ods.dream_cdr_log_local k19_ods.dream_user_profile_log_local k19_ods.dream_vlr_log_local k19_ods.dream_sms_log_local k19_ods.dream_email_log_local k19_ods.ck_hour_increament_materialized_view_local k19_ods.ck_day_increament_materialized_view_local k19_ods.ck_slow_query_log_local  k19_ods.ntc_mail_log_local_group_s_ip k19_ods.ntc_mail_log_local_group_d_ip k19_ods.ntc_mail_log_local_group_mail_from k19_ods.ntc_mail_log_local_group_mail_to"
function check(){
  for ((i=0;i<${#astana_local_host_name[*]};i++))
  do
