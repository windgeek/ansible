...
function check_system_tables(){
  ...
  do
    ...
    do
       {
         msg=`clickhouse-client -m  -h ${distribute_host_name[i]} --port=${tcp_port} --user=${user} --password=$writer -q "ATTACH TABLE $table;" 2>&1`
       }||{
            #如果不是表已经存在
            if [[ ${msg:0-17:16} != "already exists.." ]]
            then
               if [[  `clickhouse-client -m  -h ${distribute_host_name[i]} --port=${tcp_port} --user=${user} --password=$writer -q "EXISTS $table;"` == 0  ]]; then
            echo ${distribute_host_name[i]} $table doesn't exists' and now create it!
            clickhouse-client -m  -h ${distribute_host_name[i]} --port=${tcp_port} --user=${user} --password=$writer -q "${system_table_sqls[$table]}"
              fi
            fi
          }
    done
...
function astana_create_distribute_table(){
  for ((i=0;i<${#distribute_host_name[*]};i++))
  do
       echo "执行 ${ck_init_script_path}/${distribute_table_sql} 在 ${distribute_host_name[i]} 上创建分布式表"
       cat ${ck_init_script_path}/${distribute_table_sql}|while read line
        do
          {
            #create或者alter命令
            if [[ ${line:0:6} = "CREATE" ||  ${line:0:5} = "ALTER" ]]
            then
                msg=`clickhouse-client --host=${distribute_host_name[i]} --port=${tcp_port} --user=${user} --password=$writer -q "$line"  2>&1`
            fi
          }||{
            #如果error是表已经存在则不显示
            if [[ ${msg:0-17:16} != "already exists.." ]]
...
tables="k19_distribute.ntc_http_log k19_distribute.ntc_lbs_log k19_distribute.ntc_mail_log k19_distribute.ntc_radius_log k19_distribute.ntc_voip_log k19_distribute.ntc_ip_log k19_distribute.ntc_nat_log k19_distribute.dm_conv_log k19_distribute.dream_http_log k19_distribute.dream_cdr_log k19_distribute.dream_user_profile_log k19_distribute.dream_vlr_log k19_distribute.dream_sms_log k19_distribute.dream_email_log k19_distribute.ck_query_log k19_distribute.ck_parts k19_distribute.ck_all_replicas k19_distribute.replicas_tse_small_log k19_distribute.replicas_ala_small_log k19_distribute.ck_hour_increament_materialized_view k19_distribute.ck_day_increament_materialized_view k19_distribute.ck_slow_query_log k19_ods.ck_slow_query_log_local k19_distribute.ntc_ip_log_group_d_ip k19_distribute.ntc_ip_log_group_radius_account k19_distribute.ntc_http_log_group_s_ip k19_distribute.ntc_http_log_group_d_ip k19_distribute.ntc_http_log_group_radius_account k19_distribute.ntc_http_log_group_website k19_distribute.dm_conv_log_group_client_ip k19_distribute.dm_conv_log_group_server_ip k19_distribute.ntc_mail_log_group_s_ip k19_distribute.ntc_mail_log_group_d_ip k19_distribute.ntc_mail_log_group_mail_from k19_distribute.ntc_mail_log_group_mail_to"
function check(){
  for ((i=0;i<${#distribute_host_name[*]};i++))
  do
      echo "在${distribute_host_name[i]}执行本地表建表检查"
      for table in $tables
      do
        if [[  `clickhouse-client -m -d k19_distribute -h ${distribute_host_name[i]} --port=${tcp_port} --user=${user} --password=$writer -q "EXISTS $table;"` == 0  ]]; then
            echo ${distribute_host_name[i]} $table doesn't exists'
        fi
      done
  done
}
