...
curl -XPUT "http://${hostIp}:9200/_all/_settings" -H 'Content-Type: application/json' -d '
{
  "settings": {
...
curl -XPUT "http://${hostIp}:9200/_cluster/settings" -H 'Content-Type: application/json' -d '
{
	"persistent":{
...
# 修改监控模版
curl -XPUT "http://${hostIp}:9200/_template/elasticsearch_metrics" -H 'Content-Type: application/json' -d '
{
...
#修改慢查询日志
curl -XPUT "http://${hostIp}:9200/k19*/_settings" -H 'Content-Type: application/json' -d '
{
