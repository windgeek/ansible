...
#获取磁盘数量（实际数量=PDNUM+1）
declare -i PDNUM1 DEC PDNUM
PDNUM1=`/opt/MegaRAID/MegaCli/MegaCli64 -PDGetNum -aAll|grep Physical|awk '{ print $8 }'` 
DEC=1
PDNUM=$PDNUM1-$DEC
#磁盘分块后文件名规范统一化
for i in $(seq 0  ${PDNUM} )
do
...
CHECKRAIDLEVEL()
{
/opt/MegaRAID/MegaCli/MegaCli64  -LDInfo -Lall -aALL|grep -E 'Offline|Degraded' >> /dev/null 2>&1
if [ $? = 0 ]
then
echo -1
else
